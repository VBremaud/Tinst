...
def plot_detection_raies(fichier=r'\Users\Vincent\Documents\Stage J.Neveu\Programmes et prod\CTIODataJune2017 prod4\sim_20170530_060_spectrum.txt',nb_tour=1,CALSPEC=False,trigger=2,lambdamin=250,lambdamax=1150,filtre1_window=61,filtre1_order=3,moy_raies=10,demi_taille_max=40):
    ...
    for z in range(nb_tour):
        ...
        if z==0:
            ...
            print(np.array(Intensite_obs_savgol)-np.array(Intensite_obs_corr))
        ...
        while i<len(D_intensite_obss)-1:
            ...
            while len(moy)!=moy_raies*10:
                if Raies[i-j]==False:
                    moy.append(D_intensite_obss[-(i-j+1)])
                j+=1
            D_mean.append(np.mean(moy))
            D_sigma.append(np.std(moy))

            if D_intensite_obss[-(i+1)]>D_mean[-1]+trigger*D_sigma[-1]:
                k=i+demi_taille_max*10
                for j in range(i,k):
                    if D_intensite_obss[-(j+2)]-D_intensite_obss[-(j+1)]<0:
                        var_signe=1
                    if var_signe==1 and D_intensite_obss[-(j+2)]-D_intensite_obss[-(j+1)]>0:
                        break

                    if D_intensite_obss[-(j+1)]<D_mean[-1]-trigger*D_sigma[-1]:
                        if len(lambda_complet)-1>j+k-i:
                            indice=j+k-i
                        else:
                            indice=len(lambda_complet)-1
                        for v in range(j,indice):
                            if D_intensite_obss[-(v+2)]-D_intensite_obss[-(v+1)]>0:
                                if var_signe==1:
                                    var_signe=2

                            if var_signe==2 and D_intensite_obss[-(v+2)]-D_intensite_obss[-(v+1)]<0:
                                var_signe=3

                            if D_intensite_obss[-(v+1)]>D_mean[-1]-trigger*D_sigma[-1]:
                                indice=v
                                break
